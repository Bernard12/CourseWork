...
int* create( int number, int random, int* max)
{
...
void re_size(int* l, int flag, int* max)
{
    if(flag)
    {
...
void insert(int* l, int val, int pos, int* max)
{
    if( (l[0]+3)>=(*max)) re_size(l,1,max);
    if(pos<1) pos=1;
    if(pos<=l[0])
    {
        int i=l[0]+1;
...
int del(int*l, int pos, int* max)
{
    if(l[0]==0) return (INT_MIN);
    if(l[0]==pos) {l[0]--;re_size(l,0,max);return l[l[0]+1];}
    if(pos<1) pos=1;
    int res=l[pos];
